...
**벤치마크 용어:**
- **FPS 관측값**: CPU·GPU·게임·해상도·옵션·숫자 FPS와 원본 조건을 함께 보존한 하나의 성능 기록이다.
- **원문 벤치마크**: ComputerBase처럼 테스트 시스템과 차트 조건이 명시된 공개 성능 자료다.
- **원문 추천값**: 견적왕처럼 특정 견적에 대해 작성자가 제시한 해상도·옵션·FPS 안내값이다. FPS 숫자를 보존하지만 원문 벤치마크나 JHS 직접 측정으로 표현하지 않는다.
- **FPS 근거 조합**: 하나 이상의 FPS 관측값이 연결된 정규화 CPU/GPU 조합이다. 추천 구성만 있는 조합과 구분한다.
- **출처 조건**: 원본 게시물·기사·테스트 시스템·옵션·수집 시각을 재현하기 위한 provenance 묶음이다.
