  ...
  return (
    <div className="rounded-3xl border border-line bg-white p-4 shadow-soft sm:p-5">
      ...
      <div className="mt-4 grid gap-3 sm:grid-cols-2 lg:grid-cols-1">
        {combos.map((combo) => (
          <Link href={`/benchmarks?comboKey=${encodeURIComponent(combo.publicComboRef ?? combo.comboKey)}`} className="block rounded-2xl border border-line p-4 transition hover:border-brand/40 hover:bg-teal-50/30" key={combo.publicComboRef ?? combo.comboKey}>
            <p className="text-sm font-black text-slate-900">{combo.publicComboName ?? `${combo.cpuModel} + ${combo.gpuModel}`}</p>
            <div className="mt-3 flex flex-wrap gap-2 text-xs font-bold">
              ...
              <span className={`rounded-full px-2 py-1 ${combo.hasFpsEvidence ? 'bg-amber-50 text-amber-700' : 'bg-slate-100 text-slate-500'}`}>
                {combo.hasFpsEvidence ? 'FPS 근거 있음' : '성능 데이터 준비 중'}
              </span>
            </div>
          </Link>
        ))}
        {combos.length === 0 && <p className="rounded-2xl bg-slate-50 p-4 text-sm font-bold text-slate-500">수집된 조합이 없습니다. 데이터 수집 상태를 확인해 주세요.</p>}
      </div>
    </div>
  );
}
