...
ALTER TABLE `benchmark_fps_results`
  ...
  ADD UNIQUE KEY `uk_benchmark_fps` (`BENCHMARK_BUILD_ID`, `GAME_ID`, `RESOLUTION`, `OPTION_KEY`, `SOURCE_CONDITION_KEY`, `EVIDENCE_TYPE`);
...
ALTER TABLE `benchmark_combo_game_results`
  ...
  ADD COLUMN IF NOT EXISTS `SOURCE_CONDITION_KEY` VARCHAR(160) NOT NULL DEFAULT 'UNKNOWN' AFTER `OPTION_KEY`,
  ADD COLUMN IF NOT EXISTS `EVIDENCE_TYPE` VARCHAR(40) NOT NULL DEFAULT 'SOURCE_BENCHMARK' AFTER `SOURCE_CONDITION_KEY`,
  ADD UNIQUE KEY `uk_benchmark_combo_game`        (`BENCHMARK_COMBO_ID`, `GAME_ID`, `RESOLUTION`, `OPTION_KEY`, `SOURCE_CONDITION_KEY`, `EVIDENCE_TYPE`),
...
UPDATE `benchmark_combo_game_results` r
JOIN `benchmark_builds` b ON b.`BENCHMARK_COMBO_ID` = r.`BENCHMARK_COMBO_ID`
